...
public class BasicGraphEditor extends JPanel
{
	...
	public BasicGraphEditor(String appTitle, mxGraphComponent component)
	{
		...
		Thread Timer = new Thread(new Runnable(){
			synchronized public void run(){
				try {
					while(true){
						Thread.sleep(1000);
						label.setText(Statistics.getStat());
					}
		...
		JSplitPane inner = new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				new SettingsPanel(),label);
		inner.setDividerLocation(0.8);
		inner.setDividerSize(6);
		inner.setBorder(null);

		// Creates the outer split pane that contains the inner split pane and
		// the graph component on the right side of the window
		JSplitPane outer = new JSplitPane(JSplitPane.VERTICAL_SPLIT, new SettingsPanel(),label);
		outer.setOneTouchExpandable(true);
		outer.setDividerLocation(0.5);
		outer.setDividerSize(6);
		outer.setBorder(null);
		JSplitPane outer2 = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, outer,
				graphComponent);
		outer2.setOneTouchExpandable(true);
		outer2.setDividerLocation(0.8);
