/**
 * $Id: mxCurve.java,v 1.42 2010-08-05 16:31:02 david Exp $
...
public class mxCurve
{
	 ...
	 * 		as a line, parallel with the curve
	 */
	public mxLine getCurveParallel(String index, double distance)
	{
		...
		{
			return new mxLine(new mxPoint(0, 0), new mxPoint(1, 0));
		}
	}
	...
	public boolean intersectsRect(mxRectangle rect)
	{
		...
		//if (this.getBounds().intersectsRect(rect))
		if (!getBounds().getRectangle().intersects(rect.getRectangle()))
		{
			return false;
		}
		...
		if (pointsCurve != null && pointsCurve.length > 1)
		{
			...
			{
				if (rect.contains(pointsCurve[i].getX(), pointsCurve[i].getY())
						|| rect.contains(pointsCurve[i - 1].getX(),
	...
	public void updateCurve(List<mxPoint> newPoints)
	{
		...
		{
			if (!guidePoints.equals(newPoints))
			{
				pointsChanged = true;
			}
			...
			if (valid && pointsChanged
					&& newPoints.size() == guidePoints.size()
					&& newPoints.size() > 1 && guidePoints.size() > 1)
			{
				...
				for (int i = 1; i < newPoints.size(); i++)
				{
					...
					if (transX != nextTransX || transY != nextTransY)
					{
